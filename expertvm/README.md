...
mapped experts (`PinOom` only when even one expert cannot lock). `SimulatedGpuStore::new`
...
`sim` knobs (`host_func`, blocking compute, `sync_alloc`, mempool). `--max-batch N` admits N sequences per engine
