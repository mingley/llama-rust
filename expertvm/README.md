...
`sim` knobs (`host_func`, blocking compute, `sync_alloc`, mempool, `vmm_page`, pageable H2D, AccessedBy, legacy NULL). `--max-batch N` admits N sequences per engine
...
expertvm sim      trace.jsonl --capacity 2 --pageable --legacy-null
