...
`sim` knobs (`host_func`, blocking compute, `sync_alloc`, mempool, `vmm_page`, pageable H2D, AccessedBy, legacy NULL, stream priority, `graph_update`). `--max-batch N` admits N sequences per engine
...
`(device, stream)` (`graph_update_ns` instead of instantiate). Capture waits with
