...
Remote domain and fence tax returns; needs `--mem-sync-domain remote`). `--shared-mem default|four|eight` is
...
needs `--mem-sync-domain remote`). `--shared-mem default|four|eight` is kernel-node bank width
