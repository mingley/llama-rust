...
is `cudaMemAdviseSetAccessedBy` on every GPU at a managed fill: dest GEMMs
read without migrating, and replica prefetch is skipped. `--vmm` uses
...
`sim` knobs (`host_func`, blocking compute, `sync_alloc`, mempool, `vmm_page`, pageable H2D, AccessedBy, legacy NULL, stream priority, `graph_update`, `graph_clone`, `timing_events`). `--max-batch N` admits N sequences per engine
...
`(device, stream)` (`graph_update_ns` instead of instantiate). `--graph-clone`
clones a leaf capture before instantiate (`graph_clone_ns`; the src is
destroyed). Capture waits with
...
`--seq-streams` are the same `GpuStoreCfg` knobs as `expertvm sim`.
