...
`sim` knobs (`host_func`, blocking compute, `sync_alloc`, mempool, `vmm_page`, pageable H2D, AccessedBy, legacy NULL, stream priority). `--max-batch N` admits N sequences per engine
