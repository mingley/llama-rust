...
walker does not imply `--decode-priority`). `--shared-mem default|four|eight` is
...
fence tax does not flush decode ITL. Default is Default. gpu-sim allreduce
...
(Auto tax 0). `--mem-sync-domain default|remote` is decode-stream
