...
| `CachedStore` | Bounded LRU with **leases** so in-use experts cannot be evicted. `prefetch(keys)` skips unknown keys. `pin_hot` / `is_resident` / `is_leased` / `phase` / `evict` / `take_victim`. CPU `ExpertPhase` is Cold / Resident / Leased (fault-in is instant; `evict` is immediate Cold). |
