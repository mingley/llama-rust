...
`--pageable` / `--host-register` / `--host-register-mapped` / `--sync-memops` / `--memcpy-batch` / `--accessed-by` / `--legacy-null` / `--stream-priority` /
