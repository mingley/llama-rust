...
pub struct GpuStoreCfg {
    ...
    ///
    /// Bills [`gpu_sim::GpuProfile::alloc_overhead_ns`]. Hits stay the same
    /// iff [`gpu_sim::Sim::va_release_handle`] runs before
    ...
    /// Illegal with mapped, managed, [`Self::pageable`], and [`Self::memcpy_batch`].
    /// Hits stay the same. Decode identity stays pinned H2D.
    ...
    /// previous child so sibling GEMMs serialize (empty edges stay independent).
    /// Does not imply graph-build. Hits stay the same. Decode identity stays
    ...
    ///
    /// Needs [`Self::graph_piecewise`]. Later fragments depend on the previous
    /// last node so sibling GEMMs serialize (empty deps stay extra roots).
    /// Does not imply piecewise. Hits stay the same. Decode identity stays
    /// empty deps (Hyper-Q overlap when `compute_slots >= 2`).
    ...
    /// Distinct from [`Self::graph_capture_deps`] and [`Self::graph_host`].
    /// Store GEMM stays per-leaf. Decode identity stays no host nodes.
...
pub struct SimulatedGpuStore {
    ...
    /// [`GpuStoreCfg::graph_capture_host`]: walker combo captured host BETWEEN fragments (store GEMM stays per-leaf).
...
impl SimulatedGpuStore {
    ...
    /// Whether miss pages call `cuMemRetainAllocationHandle` after VMM map.
    #[must_use]
    ...
    #[must_use]
    pub fn page_vmm_handle(&self, key: ExpertKey) -> Option<MemHandleId> {
        self.pages.get(&key).and_then(|p| p.retain)
    }

    /// Whether the resident page for `key` currently holds a live VMM retain handle.
    #[must_use]
    pub fn page_is_vmm_retain(&self, key: ExpertKey) -> bool {
        self.pages
            .get(&key)
            .and_then(|p| p.retain)
            .and_then(|h| self.sim.is_handle_live(h).ok())
            .unwrap_or(false)
    }
    ...
    /// Whether walker combo capture-to-graph deps were requested (store GEMM stays per-leaf).
    #[must_use]
    pub fn graph_capture_deps(&self) -> bool {
        self.graph_capture_deps
    }

    /// Whether walker combo captured host BETWEEN fragments was requested (store GEMM stays per-leaf).
    #[must_use]
    ...
    fn migrate_vmm(
        ...
        release_vmm_handle(&mut self.sim, src_retain)?;
        if self.sim.is_resident(id, src)? {
    ...
    fn piecewise_gemm_graph(&mut self, device: DeviceId, id: AllocId) -> Result<GraphId, Error> {
        ...
        let work = self.leaf_work();
        let g = self.sim.create_graph(device, self.compute)?;
        // Store GEMM is per-leaf; capture-to-graph deps stay empty and no
        // captured `host_func` is inserted even when
        // [`GpuStoreCfg::graph_capture_deps`] or
        // [`GpuStoreCfg::graph_capture_host`] is set (walker combo only).
        self.sim
    ...
    fn finish_drop(&mut self, key: ExpertKey, page: GpuPage) -> Result<(), Error> {
        ...
        match self.mode {
            ...
            GpuFill::Vmm => {
                ...
                self.free_page_mailbox(page.device, page.mailbox)?;
                release_vmm_handle(&mut self.sim, page.retain)?;
