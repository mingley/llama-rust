...
pub(crate) struct GemmFlags {
    ...
    /// `cudaLaunchAttributeMemSyncDomainMap` collapse on grouped GEMMs.
    ///
    /// Off inherits the stream. Decode identity stays inherit-stream.
...
impl GemmFlags {
    ...
    pub(crate) fn kernel_attrs(self, id: AllocId) -> KernelAttrs {
        ...
        KernelAttrs {
            ...
            mem_sync_map: self.mem_sync_map(),
            ..KernelAttrs::default()
        }
    }
    ...
    pub(crate) fn mem_sync_map(self) -> Option<MemSyncDomainMap> {
        self.mem_sync_launch_map.then_some(collapse_mem_sync_map())
    }
...
pub(crate) fn check_mem_sync_launch_map(
    domain: MemSyncDomain,
    launch_map: bool,
) -> Result<(), Error> {
    if launch_map && domain != MemSyncDomain::Remote {
        return Err(Error::Store(
...
/// of `--shareable` here (CLI implies; `with_cfg` / `validate_sim_cfg` refuse).
pub(crate) fn check_share_ptr(share_ptr: bool, shareable: bool, ipc: bool) -> Result<(), Error> {
    if !share_ptr {
        return Ok(());
    }
    if !shareable {
        return Err(Error::Store("share-ptr needs shareable"));
    }
    if ipc {
        return Err(Error::Store("choose one of ipc, shareable"));
    }
...
/// `cudaMemPoolExportPointer` then `cudaMemPoolImportPointer`. Alias shares source HBM.
pub(crate) fn open_share_alias(
    sim: &mut Sim,
    pool: Option<PoolId>,
    id: AllocId,
    share_ptr: bool,
    ...
    let pool = pool.ok_or(Error::Store("share-ptr needs shareable"))?;
    let h = sim.pool_export_ptr(id)?;
    Ok(Some(sim.pool_import_ptr(pool, h)?))
}

/// `cudaFreeAsync` of a `cudaMemPoolImportPointer` alias. No-op when `None`.
pub(crate) fn close_share_alias(
    sim: &mut Sim,
    device: DeviceId,
    stream: StreamId,
    import: Option<AllocId>,
) -> Result<(), Error> {
    if let Some(imp) = import {
        sim.free(device, imp, stream)?;
    }
    Ok(())
}
...
pub struct SimCfg {
    ...
    /// [`Self::ipc`]. Decode identity is pool-level `--shareable` without a
    /// per-page pointer handshake. [`crate::GpuStoreCfg::share_ptr`] is the
    /// store path.
    pub share_ptr: bool,
    ...
    /// staying on home. [`Self::no_mem_prefetch`] skips fill
    /// `cudaMemPrefetchAsync` so the kernel first-touches. Hits/misses match H2D.
    ...
    ///
    /// Default managed fill Sets ReadMostly so dest prefetch replicates. This
    /// calls [`gpu_sim::MemAdvise::UnsetReadMostly`] so prefetch **moves**
    /// (`hbm_peak` stays one copy). Implies [`Self::managed`] on the CLI.
    /// Hits stay the same. Distinct from [`Self::accessed_by`]. Decode
    ...
    ///
    /// Default managed fill prefetches onto home on the copy stream. This
    /// skips that prefetch so the first GEMM first-touches on compute.
    /// Implies [`Self::managed`] on the CLI. Hits stay the same. Distinct
    /// from [`Prefetch::None`] (predictor) and [`Self::prefetch_host`].
    /// Replica dest prefetch and host-restore prefetch stay. Decode identity
    /// stays fill prefetch. [`crate::GpuStoreCfg::no_mem_prefetch`] is the
    /// store path.
    ...
    /// the previous child so sibling expert GEMMs serialize through
    /// `host_func_ns`. Does not imply graph-build or [`Self::host_func`]. Hits
    /// stay the same. Decode identity stays no host nodes in combo parents.
    /// [`crate::GpuStoreCfg::graph_host`] is the store CLI field (store GEMM
    /// stays per-leaf).
    pub graph_host: bool,
    ...
    /// returns. Decode identity stays inherit-stream.
    /// [`crate::GpuStoreCfg::mem_sync_launch_map`] is the store path.
    pub mem_sync_launch_map: bool,
...
pub(crate) fn validate_sim_cfg(cfg: &SimCfg, profile: &HardwareProfile) -> Result<(), Error> {
    ...
    if cfg.no_mem_prefetch && !cfg.managed {
    ...
    if (cfg.shareable || cfg.share_ptr)
        && (cfg.sync_alloc || cfg.ipc || cfg.mapped || cfg.managed || cfg.vmm)
    {
...
pub fn sim_replay_cfg(
    ...
    let mut args = TouchArgs {
        ...
        share_pool: imported.get(&d).copied(),
    };
...
pub(crate) struct TouchArgs {
    ...
    pub no_preferred: bool,
    /// [`SimCfg::no_mem_prefetch`]: skip fill `cudaMemPrefetchAsync` (kernel first-touch).
    pub no_mem_prefetch: bool,
    ...
    pub ipc: bool,
    /// [`SimCfg::share_ptr`]: `cudaMemPoolExportPointer` / `ImportPointer` after miss fill.
    pub share_ptr: bool,
    /// Imported sibling pool for [`Self::share_ptr`] (`cudaMemPoolImportPointer`).
    pub share_pool: Option<PoolId>,
}
...
/// Dest eviction of a ReadMostly extra copy. No-op when prefetch already moved.
pub(crate) fn drop_managed_replica(
    sim: &mut Sim,
    id: AllocId,
    device: DeviceId,
) -> Result<(), Error> {
    if !sim.is_read_mostly(id)? {
        return Ok(());
    }
    if !sim.is_resident(id, device)? {
        return Ok(());
    }
    sim.drop_managed_copy(id, device)?;
    Ok(())
}
...
impl GraphBank {
    ...
    pub(crate) fn with_mem_sync_launch_map(mut self, yes: bool) -> Self {
        self.mem_sync_launch_map = yes;
        self
    }
...
pub(crate) fn apply_misses(
    ...
    let h2d: Vec<AllocId> = filled
        ...
        .collect();
    if args.managed && !args.no_mem_prefetch {
...
pub(crate) fn reclaim_victim(
    ...
    match home {
        ...
        Some(_) => {
            ...
            drop_replica(sim, page, args.d, next_event, args)
        }
    }
}
...
fn drop_handle(
    ...
    close_share_alias(sim, page.device, page.stream, page.share)?;
...
fn drop_replica(
    ...
    close_share_alias(sim, page.device, page.stream, page.share.take())?;
    if page_is_managed(sim, page.id) {
        drop_managed_replica(sim, page.id, dst)?;
    } else if page_is_vmm(sim, page.id) {
        sim.va_unmap_range(page.id, dst, 0, args.bytes)?;
    } else {
        sim.free(dst, page.id, page.stream)?;
    }
    page.replicas.retain(|d| *d != dst);
    let pinned = !page_is_managed(sim, page.id)
        && !page_is_vmm(sim, page.id)
        && !page_is_mapped(sim, page.id);
    page.ipc = open_ipc_alias(sim, page.device, page.id, args.ipc && pinned)?;
    page.share = open_share_alias(sim, args.share_pool, page.id, args.share_ptr && pinned)?;
    Ok(())
}
...
/// Hopper collapse `{default: 0, remote: 0}`.
pub(crate) fn collapse_mem_sync_map() -> MemSyncDomainMap {
    MemSyncDomainMap {
        default: 0,
        remote: 0,
    }
...
///
/// Launch collapse overrides prefill inherit-identity so leftover prefill
/// maps Default→0 with decode Remote→0. Store leaves keyed by alloc would
/// otherwise snap back to the launch stream map.
pub(crate) fn apply_exec_mem_sync_map(
    sim: &mut Sim,
    exec: GraphId,
    device: DeviceId,
    stream: StreamId,
    launch: Option<MemSyncDomainMap>,
) -> Result<(), Error> {
    let want = match launch {
        Some(m) => m,
        None => sim.stream_mem_sync_domain_map(device, stream)?,
    };
    let (node, _) = sim.graph_unique_kernel(exec)?;
...
pub(crate) struct RemotePage {
    ...
    pub(crate) ipc: Option<AllocId>,
    /// [`RemoteFetch::share_ptr`]: live `cudaMemPoolImportPointer` alias of `id`.
    pub(crate) share: Option<AllocId>,
}
...
pub(crate) struct RemoteFetch {
    ...
    /// [`SimCfg::no_mem_prefetch`]: skip fill `cudaMemPrefetchAsync` (kernel first-touch).
    pub(crate) no_mem_prefetch: bool,
    ...
    pub(crate) ipc: bool,
    /// [`SimCfg::share_ptr`]: `cudaMemPoolExportPointer` of the home `cudaMallocAsync`.
    pub(crate) share_ptr: bool,
    /// Imported sibling pool for [`Self::share_ptr`].
    pub(crate) share_pool: Option<PoolId>,
}
...
pub(crate) fn fill_remote(
    ...
    let share = open_share_alias(sim, fetch.share_pool, id, fetch.share_ptr)?;
...
pub(crate) fn drop_remote(
    ...
    close_ipc_alias(sim, page.ipc)?;
    close_share_alias(sim, page.home, stream, page.share)?;
    if page_is_managed(sim, page.id) {
