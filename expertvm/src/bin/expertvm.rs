    ...
    I: IntoIterator<Item = String>,
{
    ...
    let mut memcpy_attr = false;
    let mut memset_fill = false;
    ...
    let mut graph_clone_parent = false;
    ...
    let mut graph_capture_host = false;
    let mut graph_enable = false;
    let mut graph_if = false;
    let mut graph_mem = false;
    let mut graph_memset = false;
    let mut graph_memcpy = false;
    let mut graph_leaf_host = false;
    ...
    while let Some(arg) = it.next() {
        ...
        match key.as_str() {
            ...
            "--memcpy-attr" => memcpy_attr = switch(&inline),
            "--memset-fill" => memset_fill = switch(&inline),
            "--accessed-by" => accessed_by = switch(&inline),
            "--no-read-mostly" => no_read_mostly = switch(&inline),
            "--no-preferred" => no_preferred = switch(&inline),
            "--no-mem-prefetch" => no_mem_prefetch = switch(&inline),
            ...
            "--graph-clone-parent" => graph_clone_parent = switch(&inline),
            ...
            "--graph-capture-host" => graph_capture_host = switch(&inline),
            "--graph-enable" => graph_enable = switch(&inline),
            "--graph-if" => graph_if = switch(&inline),
            "--graph-mem" => graph_mem = switch(&inline),
            "--graph-memset" => graph_memset = switch(&inline),
            "--graph-memcpy" => graph_memcpy = switch(&inline),
            "--graph-leaf-host" => graph_leaf_host = switch(&inline),
            ...
            "--green-ctx" => green_ctx = switch(&inline),
            "--plan-window" => {
    ...
    if vmm_page > 0 || multicast || vmm_retain || vmm_handle {
        vmm = true;
    }
    ...
    if graph_capture_host && !graph_piecewise {
        return Err("--graph-capture-host needs --graph-piecewise".into());
    }
    if graph_leaf_host && device_launch {
    ...
    if graph_if && !graph_build {
        return Err("--graph-if needs --graph-build".into());
    }
    if graph_if && device_launch {
        return Err("graph-if cannot device-launch".into());
    }
    if graph_if && graph_enable {
...
fn sim_cfg_from(cfg: &Cfg, prefetch: Prefetch, max_batch: usize) -> SimCfg {
    SimCfg {
        ...
        graph_capture_host: cfg.graph_capture_host,
        graph_enable: cfg.graph_enable,
        graph_if: cfg.graph_if,
