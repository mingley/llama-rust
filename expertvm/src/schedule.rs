...
use crate::sim_replay::{
    ...
    apply_l2_fetch, apply_misses, apply_required_cluster_width, apply_stream_mem_sync_domain,
                ...
                Touch::Miss { evicted } => {
                    ...
                    let page = fill_remote(
                        ...
                        RemoteFetch {
                            ...
                            no_read_mostly: self.cfg.no_read_mostly,
                            no_preferred: self.cfg.no_preferred,
                            no_mem_prefetch: self.cfg.no_mem_prefetch,
...
fn finish_sched(mut rt: SchedRt, rec: Rec) -> Result<SchedReplay, Error> {
    ...
    if rt.cfg.mempool_trim {
        trim_device_pools(&mut rt.sim)?;
    }
