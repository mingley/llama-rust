...
`--copy-host` / `--blocking-streams` / `--sync-alloc` / `--ipc` / `--mempool` / `--mempool-trim` / `--mempool-no-reuse` / `--mempool-max` / `--shareable` / `--share-ptr` / `--vmm-retain` / `--vmm-handle` / `--vmm-page` /
...
(exclusive compute still serializes leftover prefill). `--mapped` / `--managed` / `--vmm` choose miss-page placement
...
fn check_serve_need(n: &ServeNeed) -> Result<(), String> {
    ...
    if let Err(e) = n.plan.gpu.check_graph_host() {
        return usage_err(&e);
    }
    if let Err(e) = n.plan.gpu.check_graph_if() {
        return usage_err(&e);
    }
    if let Err(e) = n.plan.gpu.check_graph_memset() {
        return usage_err(&e);
    }
    if let Err(e) = n.plan.gpu.check_graph_memcpy() {
        return usage_err(&e);
    }
    if let Err(e) = n.plan.gpu.check_graph_leaf_host() {
        return usage_err(&e);
    }
    if let Err(e) = n.plan.gpu.check_graph_capture_deps() {
        return usage_err(&e);
    }
    if let Err(e) = n.plan.gpu.check_graph_capture_host() {
        return usage_err(&e);
    }
    if n.plan.gpu.graph_enable && n.plan.gpu.device_launch {
        return usage_err("graph-enable cannot device-launch");
    }
    if n.plan.gpu.graph_if && n.plan.gpu.device_launch {
        return usage_err("graph-if cannot device-launch");
    }
    ...
    if (n.plan.gpu.shareable || n.plan.gpu.share_ptr)
        && (n.plan.gpu.sync_alloc
            || n.plan.gpu.ipc
            || n.plan.gpu.mapped
            || n.plan.gpu.managed
            || n.plan.gpu.vmm
            || n.plan.gpu.vmm_retain
            || n.plan.gpu.vmm_handle)
    {
    ...
    if (n.plan.gpu.host_register || n.plan.gpu.host_unregister) && n.plan.gpu.host_register_mapped {
        return usage_err("choose one of --host-register, --host-register-mapped");
    }
    if (n.plan.gpu.host_register || n.plan.gpu.host_unregister)
    ...
    if let Err(e) = n.plan.gpu.check_vmm_handle() {
        return usage_err(&e);
    }
    ...
    if let Err(e) = n.plan.gpu.check_memcpy_attr() {
        return usage_err(&e);
    }
    if let Err(e) = n.plan.gpu.check_d2h_evict() {
        return usage_err(&e);
    }
    if let Err(e) = n.plan.gpu.check_d2h_pageable() {
        return usage_err(&e);
    }
    if let Err(e) = n.plan.gpu.check_memset_fill() {
        return usage_err(&e);
    }
...
mod tests {
    ...
    fn prefill_chunk_and_trace_out_need_engine() {
        ...
        let err = parse_serve_args(["m.gguf", "--graph-host"]).unwrap_err();
        assert!(err.contains("--graph-host requires --engine"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--graph-host"]).unwrap_err();
        assert!(err.contains("--graph-host requires --expert-sim"), "{err}");
        let err =
            parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--graph-host"]).unwrap_err();
        assert!(err.contains("--graph-host needs --graph-build"), "{err}");
        let err =
            parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--graph-host=1"]).unwrap_err();
        ...
        let err = parse_serve_args(["m.gguf", "--graph-memset"]).unwrap_err();
        assert!(err.contains("--graph-memset requires --engine"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--graph-memset"]).unwrap_err();
        assert!(
            err.contains("--graph-memset requires --expert-sim"),
            "{err}"
        );
        let err =
            parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--graph-memset"]).unwrap_err();
        ...
        .unwrap_err();
        assert!(err.contains("--graph-memset needs --graph-mem"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--graph-memset=1"])
            .unwrap_err();
        assert!(
        ...
        assert!(a.gpu_cfg.graph_memset);
        assert!(a.gpu_cfg.cooperative);
        let err = parse_serve_args(["m.gguf", "--graph-memcpy"]).unwrap_err();
        assert!(err.contains("--graph-memcpy requires --engine"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--graph-memcpy"]).unwrap_err();
        assert!(
            err.contains("--graph-memcpy requires --expert-sim"),
            "{err}"
        );
        let err =
            parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--graph-memcpy"]).unwrap_err();
        ...
        .unwrap_err();
        assert!(err.contains("--graph-memcpy needs --graph-mem"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--graph-memcpy=1"])
            .unwrap_err();
        assert!(
        ...
        assert!(a.gpu_cfg.graph_memcpy);
        assert!(a.gpu_cfg.cooperative);
        let err = parse_serve_args(["m.gguf", "--graph-leaf-host"]).unwrap_err();
        assert!(err.contains("--graph-leaf-host requires --engine"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--graph-leaf-host"]).unwrap_err();
        assert!(
            err.contains("--graph-leaf-host requires --expert-sim"),
            "{err}"
        );
        let err = parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--graph-leaf-host=1"])
            .unwrap_err();
        assert!(
            err.contains("--graph-leaf-host does not take a value"),
            "{err}"
        );
        let a = run(&["m.gguf", "--engine", "--expert-sim", "--graph-leaf-host"]);
        ...
        assert!(a.gpu_cfg.graph_capture_deps);
        assert!(a.gpu_cfg.pdl);
        let err = parse_serve_args(["m.gguf", "--graph-capture-host"]).unwrap_err();
        assert!(
            err.contains("--graph-capture-host requires --engine"),
            "{err}"
        );
        let err = parse_serve_args(["m.gguf", "--engine", "--graph-capture-host"]).unwrap_err();
        assert!(
            err.contains("--graph-capture-host requires --expert-sim"),
            "{err}"
        );
        let err = parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--graph-capture-host"])
            .unwrap_err();
        assert!(
        ...
        let err = parse_serve_args(["m.gguf", "--graph-if"]).unwrap_err();
        assert!(err.contains("--graph-if requires --engine"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--graph-if"]).unwrap_err();
        assert!(err.contains("--graph-if requires --expert-sim"), "{err}");
        let err =
            parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--graph-if"]).unwrap_err();
        ...
        let err = parse_serve_args(["m.gguf", "--d2h-evict"]).unwrap_err();
        assert!(err.contains("--d2h-evict requires --engine"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--d2h-evict"]).unwrap_err();
        assert!(err.contains("--d2h-evict requires --expert-sim"), "{err}");
        let a = run(&["m.gguf", "--engine", "--expert-sim", "--d2h-evict"]);
        assert!(a.gpu_cfg.d2h_evict);
        assert_eq!(a.fill, GpuFill::Pinned);
        let a = run(&["m.gguf", "--engine", "--expert-sim", "--d2h-evict", "--pdl"]);
        assert!(a.gpu_cfg.d2h_evict);
        assert!(a.gpu_cfg.pdl);
        let err =
            parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--d2h-evict=1"]).unwrap_err();
        ...
        let err = parse_serve_args(["m.gguf", "--d2h-pageable"]).unwrap_err();
        assert!(err.contains("--d2h-pageable requires --engine"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--d2h-pageable"]).unwrap_err();
        assert!(
            err.contains("--d2h-pageable requires --expert-sim"),
            "{err}"
        );
        let a = run(&["m.gguf", "--engine", "--expert-sim", "--d2h-pageable"]);
        ...
        assert!(a.gpu_cfg.d2h_pageable);
        assert!(a.gpu_cfg.pdl);
        let err = parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--d2h-pageable=1"])
            .unwrap_err();
        assert!(
        ...
        let err = parse_serve_args(["m.gguf", "--no-read-mostly"]).unwrap_err();
        assert!(err.contains("--no-read-mostly requires --engine"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--no-read-mostly"]).unwrap_err();
        assert!(
            err.contains("--no-read-mostly requires --expert-sim"),
            "{err}"
        );
        let a = run(&["m.gguf", "--engine", "--expert-sim", "--no-read-mostly"]);
        assert!(a.gpu_cfg.no_read_mostly);
        assert_eq!(a.fill, GpuFill::Managed);
        let err = parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--no-read-mostly=1"])
            .unwrap_err();
        assert!(
            err.contains("--no-read-mostly does not take a value"),
            "{err}"
        );
        let err = parse_serve_args(["m.gguf", "--no-preferred"]).unwrap_err();
        assert!(err.contains("--no-preferred requires --engine"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--no-preferred"]).unwrap_err();
        assert!(
            err.contains("--no-preferred requires --expert-sim"),
            "{err}"
        );
        let a = run(&["m.gguf", "--engine", "--expert-sim", "--no-preferred"]);
        assert!(a.gpu_cfg.no_preferred);
        assert_eq!(a.fill, GpuFill::Managed);
        let err = parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--no-preferred=1"])
            .unwrap_err();
        assert!(
            err.contains("--no-preferred does not take a value"),
            "{err}"
        );
        let err = parse_serve_args(["m.gguf", "--no-mem-prefetch"]).unwrap_err();
        assert!(err.contains("--no-mem-prefetch requires --engine"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--no-mem-prefetch"]).unwrap_err();
        assert!(
            err.contains("--no-mem-prefetch requires --expert-sim"),
            "{err}"
        );
        let a = run(&["m.gguf", "--engine", "--expert-sim", "--no-mem-prefetch"]);
        assert!(a.gpu_cfg.no_mem_prefetch);
        assert_eq!(a.fill, GpuFill::Managed);
        let err = parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--no-mem-prefetch=1"])
            .unwrap_err();
        assert!(
        ...
        let err = parse_serve_args(["m.gguf", "--memcpy-attr"]).unwrap_err();
        assert!(err.contains("--memcpy-attr requires --engine"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--memcpy-attr"]).unwrap_err();
        assert!(err.contains("--memcpy-attr requires --expert-sim"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--memcpy-attr=1"])
            .unwrap_err();
        assert!(err.contains("--memcpy-attr does not take a value"), "{err}");
        let a = run(&["m.gguf", "--engine", "--expert-sim", "--memcpy-attr"]);
        ...
        let err = parse_serve_args(["m.gguf", "--memset-fill"]).unwrap_err();
        assert!(err.contains("--memset-fill requires --engine"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--memset-fill"]).unwrap_err();
        assert!(err.contains("--memset-fill requires --expert-sim"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--memset-fill=1"])
            .unwrap_err();
        assert!(err.contains("--memset-fill does not take a value"), "{err}");
        let a = run(&["m.gguf", "--engine", "--expert-sim", "--memset-fill"]);
        ...
        let err = parse_serve_args(["m.gguf", "--copy-host"]).unwrap_err();
        assert!(err.contains("--copy-host requires --engine"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--copy-host"]).unwrap_err();
        assert!(err.contains("--copy-host requires --expert-sim"), "{err}");
        let err =
            parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--copy-host=1"]).unwrap_err();
        assert!(err.contains("--copy-host does not take a value"), "{err}");
        let a = run(&["m.gguf", "--engine", "--expert-sim", "--copy-host"]);
        assert!(a.gpu_cfg.copy_host);
        assert!(!a.gpu_cfg.host_func);
        let a = run(&["m.gguf", "--engine", "--expert-sim", "--copy-host", "--pdl"]);
        assert!(a.gpu_cfg.copy_host);
        assert!(a.gpu_cfg.pdl);
        let a = run(&[
            "m.gguf",
        ...
        let err = parse_serve_args(["m.gguf", "--host-unregister"]).unwrap_err();
        assert!(err.contains("--host-unregister requires --engine"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--host-unregister"]).unwrap_err();
        assert!(
            err.contains("--host-unregister requires --expert-sim"),
            "{err}"
        );
        let a = run(&["m.gguf", "--engine", "--expert-sim", "--host-unregister"]);
        assert!(a.gpu_cfg.host_unregister);
        assert!(a.gpu_cfg.host_register);
        assert!(a.gpu_cfg.pageable);
        let err = parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--host-unregister=1"])
            .unwrap_err();
        assert!(
        ...
        let err = parse_serve_args(["m.gguf", "--share-ptr"]).unwrap_err();
        assert!(err.contains("--share-ptr requires --engine"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--share-ptr"]).unwrap_err();
        assert!(err.contains("--share-ptr requires --expert-sim"), "{err}");
        let a = run(&["m.gguf", "--engine", "--expert-sim", "--share-ptr"]);
        assert!(a.gpu_cfg.share_ptr);
        assert!(a.gpu_cfg.shareable);
        assert!(a.gpu_cfg.mempool);
        let err =
            parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--share-ptr=1"]).unwrap_err();
        assert!(err.contains("--share-ptr does not take a value"), "{err}");
        let a = run(&["m.gguf", "--engine", "--expert-sim", "--share-ptr", "--pdl"]);
        assert!(a.gpu_cfg.share_ptr);
        assert!(a.gpu_cfg.pdl);
        let err = parse_serve_args([
            "m.gguf",
        ...
        .unwrap_err();
        assert!(err.contains("shareable needs cudaMallocAsync"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--share-ptr", "--ipc"])
            .unwrap_err();
        assert!(
            err.contains("choose one of --ipc, --shareable")
                || err.contains("shareable needs cudaMallocAsync"),
        ...
        let err = parse_serve_args(["m.gguf", "--vmm-retain"]).unwrap_err();
        assert!(err.contains("--vmm-retain requires --engine"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--vmm-retain"]).unwrap_err();
        assert!(err.contains("--vmm-retain requires --expert-sim"), "{err}");
        let a = run(&["m.gguf", "--engine", "--expert-sim", "--vmm-retain"]);
        assert!(a.gpu_cfg.vmm_retain);
        assert_eq!(a.fill, GpuFill::Vmm);
        let err =
            parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--vmm-retain=1"]).unwrap_err();
        ...
        .unwrap_err();
        assert!(err.contains("choose one of mapped, managed, vmm"), "{err}");
        let err = parse_serve_args(["m.gguf", "--vmm-handle"]).unwrap_err();
        assert!(err.contains("--vmm-handle requires --engine"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--vmm-handle"]).unwrap_err();
        assert!(err.contains("--vmm-handle requires --expert-sim"), "{err}");
        let a = run(&["m.gguf", "--engine", "--expert-sim", "--vmm-handle"]);
        assert!(a.gpu_cfg.vmm_handle);
        assert_eq!(a.fill, GpuFill::Vmm);
        let err =
            parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--vmm-handle=1"]).unwrap_err();
        ...
        assert!(a.gpu_cfg.decode_priority);
        assert!(a.gpu_cfg.stream_priority);
        let err = parse_serve_args(["m.gguf", "--green-ctx"]).unwrap_err();
        assert!(err.contains("--green-ctx requires --engine"), "{err}");
        let err = parse_serve_args(["m.gguf", "--engine", "--green-ctx"]).unwrap_err();
        assert!(err.contains("--green-ctx requires --expert-sim"), "{err}");
        let a = run(&["m.gguf", "--engine", "--expert-sim", "--green-ctx"]);
