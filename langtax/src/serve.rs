...
mod tests {
    ...
    fn prefill_chunk_and_trace_out_need_engine() {
        ...
        assert!(!a.gpu_cfg.decode_priority);
        let err = parse_serve_args(["m.gguf", "--engine", "--expert-sim", "--wait-value=1"])
            .unwrap_err();
