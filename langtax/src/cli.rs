...
`--graph-set-params` / `--graph-clone` / `--graph-build` / `--graph-piecewise` / `--graph-enable` / `--graph-mem` / `--graph-auto-free` / `--graph-mem-trim` / `--timing-events` / `--host-func` / `--blocking-streams` /
`--sync-alloc` / `--mempool` / `--mempool-trim` / `--mempool-no-reuse` / `--shareable` / `--vmm-page` / `--pageable` / `--host-register` / `--memcpy-batch` / `--accessed-by` /
`--legacy-null` / `--stream-priority` / `--seq-streams` / `--kv-sim` /
`--kv-bytes` / `--decode-priority` / `--cooperative` / `--pdl` / `--l2-persist` / `--cluster` / `--preferred-cluster` / `--cluster-spread` / `--max-shared` / `--non-portable-cluster` / `--sync-policy` / `--mem-sync-domain` / `--shared-mem` / `--portable-cluster` / `--optin-shared` / `--dynamic-shared` / `--portable-shared` / `--nvlink-util` / `--device-launch` / `--device-updatable` / `--kernel-priority` / `--launch-completion` / `--programmatic-event` / `--stream-attach` / `--managed-host` / `--wait-value` / `--multicast` / `--compute-slots` / `--decode-sms` match `GpuStoreCfg` / `expertvm sim`. `--kv-sim` bills interned
...
without `--cluster` of at least 2; legal with `--pdl` and `--cooperative`). `--max-shared`
is `cudaLaunchAttributePreferredSharedMemoryCarveout` MaxShared: occupies
every Hyper-Q slot (legal with `--pdl` and `--cooperative`). `--non-portable-cluster`
...
on decode-stream ITL when `--decode-priority`; Auto tax 0). `--mem-sync-domain
...
`1000 / shared_mem_*_permille`). `--portable-cluster default|portable|non-portable`
...
fn check_engine_sim_opts(n: &EngineSimNeed) -> Result<(), String> {
    ...
    if n.gpu.memcpy_batch && (n.gpu.pageable || n.gpu.host_register || n.gpu.sync_alloc || n.gpu.mapped || n.gpu.managed) {
...
mod tests {
    ...
    fn engine_kv_sim_flags() {
        ...
        match parse_engine_args(["m.gguf", "--expert-sim", "--stream-attach"]).expect("stream-attach")
        {
            EngineCmd::Run(a) => {
        ...
        assert!(err.contains("--stream-attach does not take a value"), "{err}");
        let err = parse_engine_args([
            "m.gguf",
            "--expert-sim",
            "--stream-attach",
        ...
        assert!(err.contains("--managed-host does not take a value"), "{err}");
        ...
        assert!(err.contains("--host-register does not take a value"), "{err}");
        let err = parse_engine_args(["m.gguf", "--expert-sim", "--memcpy-batch", "--host-register"])
            .unwrap_err();
        assert!(
            err.contains("memcpy-batch needs async pinned/vmm H2D"),
            "{err}"
        ...
        let err = parse_engine_args(["m.gguf", "--expert-sim", "--host-register", "--managed"])
            .unwrap_err();
        assert!(
            err.contains("host-register needs pinned/vmm H2D"),
            "{err}"
        );
