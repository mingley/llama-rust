...
`--sync-alloc` / `--mempool` / `--mempool-trim` / `--mempool-no-reuse` / `--shareable` / `--vmm-page` / `--pageable` / `--memcpy-batch` / `--accessed-by` /
...
fn check_engine_sim_opts(n: &EngineSimNeed) -> Result<(), String> {
    ...
    if n.gpu.memcpy_batch && (n.gpu.pageable || n.gpu.sync_alloc || n.gpu.mapped || n.gpu.managed) {
