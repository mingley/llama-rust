...
`--sync-alloc` / `--mempool` / `--mempool-trim` / `--mempool-no-reuse` / `--mempool-max` / `--shareable` / `--vmm-page` / `--pageable` / `--host-register` / `--host-register-mapped` / `--sync-memops` / `--device-sync-memops` / `--memcpy-batch` / `--memcpy-during` / `--memcpy-any` / `--memset-fill` / `--accessed-by` / `--no-read-mostly` /
`--no-preferred` / `--legacy-null` / `--stream-priority` / `--seq-streams` / `--kv-sim` /
