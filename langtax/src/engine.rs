...
//! `mempool_trim`, `mempool_no_reuse`, `mempool_max`, shareable POSIX-FD IPC, ipc cudaMalloc IPC, `vmm_page`, pageable H2D, `host_register`, `host_unregister`, `host_register_mapped`, `sync_memops`, `device_sync_memops`, `memcpy_batch`, `memcpy_during`, `memcpy_any`, `memcpy_attr`, `memset_fill`, `SetAccessedBy`, legacy NULL, stream priority,
...
//! Default inherits that occupancy. `--max-l1` is launch MaxL1 (needs
//! `--func-max-shared`; restores Hyper-Q overlap; exclusive with `--max-shared`). `--non-portable-cluster`
...
//! Auto tax 0). `--device-sync-policy` auto|spin|yield|blocking is
//! `cudaSetDeviceFlags` SCHEDULE_* (Auto streams inherit; explicit `--sync-policy`
//! wins). `--shared-mem` default|four|eight is kernel-node bank width
...
//! waits the PDL trigger; illegal with `--device-launch`). `--stream-attach` is
//! `cudaStreamAttachMemAsync` Single on managed experts (prefetch on compute;
//! implies `--managed`; illegal with `--seq-streams`). `--managed-host` is
...
//! / `--memcpy-batch`; identity stays async pinned H2D). `--device-sync-memops` is
//! `cudaSetDeviceFlags` SyncMemops (host-sync memcpy on that GPU; illegal with
//! `--mapped` / `--memcpy-batch`; identity stays async pinned H2D). `--wait-value` is
...
//! `--memcpy-batch`; the batch API waits those copies; identity stays Stream order). `--memcpy-any` is
//! `cudaMemcpySrcAccessOrderAny` on `--memcpy-batch` prefetch (needs `--memcpy-batch`; empty deps;
//! no API wait; not with `--memcpy-during`). `--memcpy-attr` is
//! `cudaMemcpyWithAttributesAsync` DuringApiCall on demand pinned/VMM miss H2D
//! (the API waits that copy; identity stays `memcpy_pinned_to_device`; does not
//! imply `--memcpy-batch`; not with `--mapped` / `--managed` / `--pageable` /
//! `--memset-fill` / `--sync-alloc` / `--sync-memops` / `--device-sync-memops`).
//! Decode
...
mod tests {
    ...
    #[test]
    fn engine_gpu_memcpy_any_keeps_greedy_identity() {
        let bytes = tiny_qwen3moe_2layer_gguf();
        let iso = mixed_gpu_decode_itl_on(bytes.clone(), false, None, GpuStoreCfg::default());
        let any = mixed_gpu_decode_itl_on(
            bytes,
            false,
            None,
            GpuStoreCfg {
    ...
    #[test]
    fn engine_gpu_memcpy_attr_keeps_greedy_identity() {
        let bytes = tiny_qwen3moe_2layer_gguf();
        let iso = mixed_gpu_decode_itl_on(bytes.clone(), false, None, GpuStoreCfg::default());
        let attr = mixed_gpu_decode_itl_on(
            bytes,
            false,
            None,
            GpuStoreCfg {
        ...
        assert_eq!(iso.2, 4);
        assert_eq!(attr.2, 4);
        assert_eq!(iso.4, attr.4, "memcpy-attr must keep greedy identity");
    }

    #[test]
    fn engine_gpu_d2h_evict_keeps_greedy_identity() {
        let bytes = tiny_qwen3moe_2layer_gguf();
        let iso = mixed_gpu_decode_itl_on(bytes.clone(), false, None, GpuStoreCfg::default());
    ...
    #[test]
    fn engine_gpu_ipc_keeps_greedy_identity() {
        let bytes = tiny_qwen3moe_2layer_gguf();
        let iso = mixed_gpu_decode_itl_on(bytes.clone(), false, None, GpuStoreCfg::default());
        let ipc = mixed_gpu_decode_itl_on(
            bytes,
            false,
            None,
            GpuStoreCfg {
        ...
        assert_eq!(iso.2, 4);
        assert_eq!(ipc.2, 4);
        assert_eq!(iso.4, ipc.4, "ipc must keep greedy identity");
    }
    ...
    #[test]
    fn engine_gpu_cluster_serializes_mixed_wall() {
        let bytes = tiny_qwen3moe_2layer_gguf();
        let profile = HardwareProfile::parse("gpus=1\nfp16_flops=1000000\ncopy_engines=2\n")
            .expect("slow gemm profile");
        let pri = GpuStoreCfg {
        ...
        assert_eq!(
            overlap.4, serial.4,
            "cluster launch must keep greedy identity"
        );
        assert!(
            overlap.1.wall_ns < serial.1.wall_ns,
    ...
    #[test]
    fn engine_gpu_cluster_must_set_matches_cluster_wall() {
        let bytes = tiny_qwen3moe_2layer_gguf();
        let profile = HardwareProfile::parse("gpus=1\nfp16_flops=1000000\ncopy_engines=2\n")
            .expect("slow gemm profile");
        let pri = GpuStoreCfg {
        ...
        let must = mixed_gpu_decode_itl_at(
            bytes,
            false,
            None,
            GpuStoreCfg {
        ...
        assert_eq!(
            cluster.4, must.4,
            "ClusterDimMustBeSet must keep greedy identity"
        );
        assert_eq!(
            cluster.1.wall_ns.saturating_add(1),
            must.1.wall_ns,
            "ClusterDimMustBeSet occupancy matches --cluster; SetAttribute is +1 ns; cluster={} must={} cluster_line={} must_line={}",
            cluster.1.wall_ns,
            must.1.wall_ns,
            cluster.1.line(),
            must.1.line()
    ...
    #[test]
    fn engine_gpu_max_shared_serializes_mixed_wall() {
        let bytes = tiny_qwen3moe_2layer_gguf();
        let profile = HardwareProfile::parse("gpus=1\nfp16_flops=1000000\ncopy_engines=2\n")
            .expect("slow gemm profile");
        let pri = GpuStoreCfg {
        ...
        assert_eq!(
            overlap.4, serial.4,
            "MaxShared carveout launch must keep greedy identity"
        );
        assert!(
            overlap.1.wall_ns < serial.1.wall_ns,
            "MaxShared carveout must not overlap leftover prefill with decode; overlap={} serial={} overlap_line={} serial_line={}",
            overlap.1.wall_ns,
            serial.1.wall_ns,
            overlap.1.line(),
            serial.1.line()
    ...
    #[test]
    fn engine_gpu_func_max_shared_serializes_mixed_wall() {
        let bytes = tiny_qwen3moe_2layer_gguf();
        let profile = HardwareProfile::parse("gpus=1\nfp16_flops=1000000\ncopy_engines=2\n")
            .expect("slow gemm profile");
        let pri = GpuStoreCfg {
        ...
        assert_eq!(
            overlap.4, serial.4,
            "func MaxShared carveout must keep greedy identity"
        );
        assert!(
            overlap.1.wall_ns < serial.1.wall_ns,
            "func MaxShared carveout must not overlap leftover prefill with decode; overlap={} serial={} overlap_line={} serial_line={}",
            overlap.1.wall_ns,
            serial.1.wall_ns,
            overlap.1.line(),
            serial.1.line()
    ...
    #[test]
    fn engine_gpu_max_l1_restores_mixed_wall() {
        let bytes = tiny_qwen3moe_2layer_gguf();
        let profile = HardwareProfile::parse("gpus=1\nfp16_flops=1000000\ncopy_engines=2\n")
            .expect("slow gemm profile");
        let pri = GpuStoreCfg {
        ...
        assert_eq!(
            serial.4, restored.4,
            "launch MaxL1 must keep greedy identity"
        );
        assert!(
            restored.1.wall_ns < serial.1.wall_ns,
            "launch MaxL1 must restore leftover prefill overlap; restored={} serial={} restored_line={} serial_line={}",
            restored.1.wall_ns,
            serial.1.wall_ns,
            restored.1.line(),
            serial.1.line()
    ...
    #[test]
    fn engine_gpu_l2_ratio_partial_is_slower_than_full() {
        let bytes = tiny_qwen3moe_2layer_gguf();
        let profile = HardwareProfile::parse(
            "gpus=1\nfp16_flops=1000000000000000\nlaunch_overhead_ns=1\nhbm_bps=1000000000\nl2_persist_hit_permille=1000\n",
        )
        ...
        let half = mixed_gpu_decode_itl_at(
            bytes.clone(),
            false,
            None,
            GpuStoreCfg {
        ...
        let full = mixed_gpu_decode_itl_at(
            bytes,
            false,
            None,
            GpuStoreCfg {
        ...
        assert_eq!(half.2, 4);
        assert_eq!(full.2, 4);
        assert_eq!(persist.4, half.4, "l2-ratio must keep greedy identity");
        assert_eq!(persist.4, full.4, "l2-ratio 1000 must keep greedy identity");
        assert_eq!(
            persist.1.wall_ns, full.1.wall_ns,
            "hitRatio 1000 must match persist; persist={} ratio1000={} persist_line={} full_line={}",
            persist.1.wall_ns,
            full.1.wall_ns,
            persist.1.line(),
            full.1.line()
        ...
        assert!(
            half.1.wall_ns > persist.1.wall_ns,
            "hitRatio 500 must bill more HBM than full persist; half={} full={} half_line={} persist_line={}",
            half.1.wall_ns,
            persist.1.wall_ns,
            half.1.line(),
            persist.1.line()
    ...
    #[test]
    fn engine_gpu_graph_capture_deps_keeps_greedy_identity() {
        let bytes = tiny_qwen3moe_2layer_gguf();
        let profile = HardwareProfile::parse(
            "gpus=1\nfp16_flops=1000000\ngraph_instantiate_ns=1\ngraph_upload_ns=1\ngraph_launch_ns=1\nlaunch_overhead_ns=1\ncopy_engines=2\n",
        )
    ...
    #[test]
    fn engine_gpu_graph_host_keeps_greedy_identity() {
        let bytes = tiny_qwen3moe_2layer_gguf();
        let profile = HardwareProfile::parse(
            "gpus=1\nfp16_flops=1000000\ngraph_instantiate_ns=1\ngraph_upload_ns=1\ngraph_launch_ns=1\nlaunch_overhead_ns=1\ncopy_engines=2\n",
        )
        ...
        assert_eq!(bld.2, 4);
        assert_eq!(host.2, 4);
        assert_eq!(bld.4, host.4, "graph-host must keep greedy identity");
    }

    #[test]
    fn engine_gpu_graph_memset_keeps_greedy_identity() {
        let bytes = tiny_qwen3moe_2layer_gguf();
        let profile = HardwareProfile::parse(
            "gpus=1\nfp16_flops=1000000\ngraph_instantiate_ns=1\ngraph_upload_ns=1\ngraph_launch_ns=1\nlaunch_overhead_ns=1\ncopy_engines=2\n",
        )
        .expect("graph profile");
        let mem = mixed_gpu_decode_itl_at(
            bytes.clone(),
            false,
            None,
            GpuStoreCfg {
        ...
        let memset = mixed_gpu_decode_itl_at(
            bytes,
            false,
            None,
            GpuStoreCfg {
        ...
        assert_eq!(mem.2, 4);
        assert_eq!(memset.2, 4);
        assert_eq!(mem.4, memset.4, "graph-memset must keep greedy identity");
    }

    #[test]
    fn engine_gpu_graph_memcpy_keeps_greedy_identity() {
        let bytes = tiny_qwen3moe_2layer_gguf();
        let profile = HardwareProfile::parse(
            "gpus=1\nfp16_flops=1000000\ngraph_instantiate_ns=1\ngraph_upload_ns=1\ngraph_launch_ns=1\nlaunch_overhead_ns=1\ncopy_engines=2\n",
        )
        ...
        let memcpy = mixed_gpu_decode_itl_at(
            bytes,
            false,
            None,
            GpuStoreCfg {
    ...
    #[test]
    fn engine_gpu_portable_shared_keeps_greedy_identity() {
        let bytes = tiny_qwen3moe_2layer_gguf();
        let profile = HardwareProfile::parse(
    ...
    #[test]
    fn engine_gpu_device_sync_policy_taxes_decode_itl() {
        let bytes = tiny_qwen3moe_2layer_gguf();
        let profile = HardwareProfile::parse(
            "gpus=1\nhost_sync_blocking_ns=10000\nfp16_flops=1000000\ncopy_engines=2\n",
        )
        ...
        assert_eq!(
            auto.4, block.4,
            "device BlockingSync must keep greedy identity"
        );
        assert!(
            block.0 > auto.0,
            "device BlockingSync must inflate decode-stream ITL; auto={} block={}",
            auto.0,
            block.0
        );
    }
    ...
    #[test]
    fn engine_gpu_shared_mem_keeps_greedy_identity() {
        let bytes = tiny_qwen3moe_2layer_gguf();
        let profile = HardwareProfile::parse(
        ...
        assert_eq!(
            def.4, eight.4,
            "EightByte shared-mem must keep greedy identity"
        );
        assert!(
