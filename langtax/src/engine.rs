...
//! `mempool_trim`, `mempool_no_reuse`, shareable POSIX-FD IPC, `vmm_page`, pageable H2D, `host_register`, `host_register_mapped`, `sync_memops`, `memcpy_batch`, `SetAccessedBy`, legacy NULL, stream priority,
...
//! / `--memcpy-batch`; identity stays async pinned H2D). `--wait-value` is
...
//! portable-shared / nvlink-util off / inherit-stream priority / no launch-completion event / no programmatic event / Global managed attach / events copy-ready / no mempool trim / opportunistic reuse / free_sync managed evict / cudaHostAllocMapped not HostRegisterMapped / async memcpy not SyncMemops).
