...
mod tests {
    ...
    #[test]
    fn engine_gpu_share_ptr_keeps_greedy_identity() {
        let bytes = tiny_qwen3moe_2layer_gguf();
        let iso = mixed_gpu_decode_itl_on(bytes.clone(), false, None, GpuStoreCfg::default());
        let share = mixed_gpu_decode_itl_on(
            bytes,
            false,
            None,
            GpuStoreCfg {
        ...
        assert_eq!(iso.2, 4);
        assert_eq!(share.2, 4);
        assert_eq!(iso.4, share.4, "share-ptr must keep greedy identity");
    }
