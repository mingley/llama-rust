...
use llama_rust::{
    ...
    tiny_llama4_gguf, tiny_llama_gguf, tiny_mixtral_gguf, tiny_qwen2_gguf, tiny_qwen3_gguf,
    write_gguf, write_gguf_with_kv, GgmlType, InferArgs, InferCmd, Kv, Llama, ServeCmd,
    TensorWrite, Tokenizer, BIN_USAGE, INFER_USAGE, QK8_0, QK_K, SERVE_USAGE,
};
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cmd.as_str() {
        ...
        "write-tiny-mixtral" => {
            let path = args.next().ok_or("write-tiny-mixtral <path>")?;
