// Q8_0 GEMV: llama.cpp ARM NEON nrc=1 kernel, lifted 1:1.
// Measures C language tax vs the Rust twin in q8_gemv.rs.
#include <arm_neon.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
static float vec_dot_q8_0_scalar(int n, const block_q8_0 *x, const block_q8_0 *y) {
    const int nb = n / QK8_0;
    float sumf = 0;
...
int main(void) {
    ...
    if (!W || !x || !y) { perror("alloc"); return 1; }
    ...
    float ref = vec_dot_q8_0_scalar(K, W, x);
    float got = vec_dot_q8_0_neon(K, W, x);
    if (fabsf(ref - got) > 1e-2f * (1.0f + fabsf(ref))) {
        fprintf(stderr, "mismatch scalar=%.6f neon=%.6f\n", ref, got);
        return 2;
    }
    ...
    double bytes = (double)niter * M * (nb * sizeof(block_q8_0) + nb * sizeof(block_q8_0));
    // Weight-dominant decode accounting: only W traffic (x is reused).
    ...
    printf("weight_GiB/s=%.2f  (W+x GiB/s=%.2f)\n", wbytes / sec / (1 << 30), bytes / sec / (1 << 30));
    printf("us_per_row=%.3f sink=%.4f check=%.4f\n", (sec * 1e6) / (niter * M), sink, y[0]);
    return 0;
}
