...
See [STATUS.md](STATUS.md) for what shipped, what is not started, and the remaining list.
...
./target/release/gguf_gemv help
./target/release/gguf_gemv write-tiny tiny-llama.gguf
./target/release/gguf_gemv infer tiny-llama.gguf
./target/release/gguf_gemv infer tiny-llama.gguf --prompt ab --n-predict 2
...
`infer <path>` with no flags is still `prompt=ab` `n_predict=2`. `--prompt` / `-p`, `--n-predict` / `-n`, and `--max-seq` are parsed in-crate (no clap). Sampling is still greedy. `--max-seq` is an optional KV length; default remains prompt tokens + `n_predict` + 1.
...
`gguf_gemv infer <gguf> [--prompt TEXT] [--n-predict N] [--max-seq N]`. Flags may appear before or after the path; `--flag=value` works.

Apple M4 Pro. Two consecutive `infer` launches on the writer-built tiny Llama GGUF (defaults `ab` / 2):
