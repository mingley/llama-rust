...
Pure-safe Rust GGUF v3 Llama-family **prompt → text**. No llama.cpp bind, no C GGML snapshot. Default `simd` is the only `unsafe`; `--no-default-features` restores `forbid(unsafe_code)`.

See [PLAN.md](PLAN.md) for the research trajectory (`expertvm`, GPU-systems VM, MoE traces). Full ChatGPT share extract: [docs/chatgpt-share-6a920fe1/](docs/chatgpt-share-6a920fe1/). See [STATUS.md](STATUS.md) for what shipped.
