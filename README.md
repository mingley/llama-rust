...
`cargo run -p llama-rust --example session` runs the writer-built Qwen3MoE tiny through CachedStore. `cargo run -p llama-rust --example engine` runs two sequences on `Engine` (chunked prefill, intern hits).
