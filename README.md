...
This is the **correctness laboratory** for the research stack. It answers “what does this model mean, and is the math right?” against an independent oracle and llama.cpp greedy on real checkpoints. The other workspace crates answer “where are the weights, and what would that cost?”

**Live agent plan:** [CIVILIZATION.md](CIVILIZATION.md) (start at item 1). [PLAN.md](PLAN.md) is the historical research plan and CUDA catalog — not the default next `mem_*` wrap.
...
See [STATUS.md](STATUS.md) for what shipped. Full ChatGPT share extract: [docs/chatgpt-share-6a920fe1/](docs/chatgpt-share-6a920fe1/). Subsequent agents execute [CIVILIZATION.md](CIVILIZATION.md), not the next CUDA identity wrap.
...
./target/release/gguf_gemv write-tiny-gemma2 tiny-gemma2.gguf
./target/release/gguf_gemv infer tiny-gemma2.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-gemma3 tiny-gemma3.gguf
./target/release/gguf_gemv infer tiny-gemma3.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-gemma3n tiny-gemma3n.gguf
./target/release/gguf_gemv infer tiny-gemma3n.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-gemma4 tiny-gemma4.gguf
./target/release/gguf_gemv infer tiny-gemma4.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-gemma4-moe tiny-gemma4-moe.gguf
./target/release/gguf_gemv infer tiny-gemma4-moe.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-gemma4-ple tiny-gemma4-ple.gguf
./target/release/gguf_gemv infer tiny-gemma4-ple.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-gemma4-moe-ple tiny-gemma4-moe-ple.gguf
./target/release/gguf_gemv infer tiny-gemma4-moe-ple.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-gemma4-moe-fused tiny-gemma4-moe-fused.gguf
./target/release/gguf_gemv infer tiny-gemma4-moe-fused.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-gemma4-moe-fused-ple tiny-gemma4-moe-fused-ple.gguf
./target/release/gguf_gemv infer tiny-gemma4-moe-fused-ple.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-gemma4-shared-kv tiny-gemma4-shared-kv.gguf
./target/release/gguf_gemv infer tiny-gemma4-shared-kv.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-gemma4-mixed-hd tiny-gemma4-mixed-hd.gguf
./target/release/gguf_gemv infer tiny-gemma4-mixed-hd.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-gemma4-swa-base tiny-gemma4-swa-base.gguf
./target/release/gguf_gemv infer tiny-gemma4-swa-base.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-gemma4-no-wv tiny-gemma4-no-wv.gguf
./target/release/gguf_gemv infer tiny-gemma4-no-wv.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-gemma4-out-scale tiny-gemma4-out-scale.gguf
./target/release/gguf_gemv infer tiny-gemma4-out-scale.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-gemma4-rope-freqs tiny-gemma4-rope-freqs.gguf
./target/release/gguf_gemv infer tiny-gemma4-rope-freqs.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-gemma4-moe-down-s tiny-gemma4-moe-down-s.gguf
./target/release/gguf_gemv infer tiny-gemma4-moe-down-s.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-gemma4-moe-gate-up-s tiny-gemma4-moe-gate-up-s.gguf
./target/release/gguf_gemv infer tiny-gemma4-moe-gate-up-s.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-llama-ffn-down-s tiny-llama-ffn-down-s.gguf
./target/release/gguf_gemv infer tiny-llama-ffn-down-s.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-llama-ffn-gate-s tiny-llama-ffn-gate-s.gguf
./target/release/gguf_gemv infer tiny-llama-ffn-gate-s.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-llama-ffn-up-s tiny-llama-ffn-up-s.gguf
./target/release/gguf_gemv infer tiny-llama-ffn-up-s.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-nvfp4-output-s tiny-nvfp4-output-s.gguf
./target/release/gguf_gemv infer tiny-nvfp4-output-s.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-bloom-attn-qkv-s tiny-bloom-attn-qkv-s.gguf
./target/release/gguf_gemv infer tiny-bloom-attn-qkv-s.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-llama-attn-q-s tiny-llama-attn-q-s.gguf
./target/release/gguf_gemv infer tiny-llama-attn-q-s.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-llama-attn-out-s tiny-llama-attn-out-s.gguf
./target/release/gguf_gemv infer tiny-llama-attn-out-s.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-llama-attn-k-s tiny-llama-attn-k-s.gguf
./target/release/gguf_gemv infer tiny-llama-attn-k-s.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-llama-attn-v-s tiny-llama-attn-v-s.gguf
./target/release/gguf_gemv infer tiny-llama-attn-v-s.gguf -p ab -n 2
...
./target/release/gguf_gemv write-tiny-qwen2moe-ffn-down-shexp-s tiny-qwen2moe-ffn-down-shexp-s.gguf
./target/release/gguf_gemv infer tiny-qwen2moe-ffn-down-shexp-s.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-qwen2moe-ffn-gate-shexp-s tiny-qwen2moe-ffn-gate-shexp-s.gguf
./target/release/gguf_gemv infer tiny-qwen2moe-ffn-gate-shexp-s.gguf -p ab -n 2
./target/release/gguf_gemv write-tiny-qwen2moe-ffn-up-shexp-s tiny-qwen2moe-ffn-up-shexp-s.gguf
./target/release/gguf_gemv infer tiny-qwen2moe-ffn-up-shexp-s.gguf -p ab -n 2
...
./target/release/gguf_gemv write-tiny-bloom tiny-bloom.gguf
./target/release/gguf_gemv infer tiny-bloom.gguf -p ab -n 2
...
`trace` is the same greedy path plus an opt-in MoE access log (`ExpertAccess` JSONL). `--out FILE` is required. After decode it prints the `expertvm replay` table (`--capacity`, default 8). Tracing does not change generated tokens. Workspace crates: [`expertvm/`](expertvm/), [`gpu-sim/`](gpu-sim/), [`infer-bench/`](infer-bench/). Live plan: [`CIVILIZATION.md`](CIVILIZATION.md). Catalog: [`PLAN.md`](PLAN.md).

`serve` binds `127.0.0.1:8080` (override with `--bind HOST:PORT`; host must be `127.0.0.1` or `localhost`). Default: one HTTP/1.1 request at a time, `POST /generate` with `{"prompt":"..."}` and optional `n_predict`, response `{"generated":"...","prefix_hit":N,"page_hits":N}`. `POST /v1/completions` and `POST /v1/chat/completions` are the same greedy path with an OpenAI `choices` envelope (`max_tokens` aliases `n_predict`; `text` / `message.content` is the completion; completions `echo` includes the prompt in `text`; `n` must be 1). `GET /v1/models` and `GET /v1/models/{id}` list/retrieve `--model-id` (default: GGUF path stem). `GET /health` is `{"status":"ok"}`. `GET /metrics` is `{"engine":false}` on the default loop, or Engine counters with `--engine`. `POST /tokenize` returns `{"tokens":[...],"count":N}` of the ids generate would prefill (`prompt` or `messages`; `add_special_tokens`, default true, inserts BOS when the tokenizer's `add_bos` is set). `POST /detokenize` takes `{"tokens":[...]}` and returns `{"text":"..."}`. HTTP/1.1 keep-alive is on unless the client sends `Connection: close`. The listener keeps one KV cache and reuses a matching token prefix (`Llama::prompt` / vLLM Automatic Prefix Caching on this engine). `--n-ctx` sizes that cache. `--kv-page N` stores KV in interned blocks of `N` tokens (`Llama::new_paged_cache`); logits match the dense path. `--engine` admits concurrent `POST /generate` onto one `Engine` so prefills GEMM together (`--max-seqs`, default 4; `--n-ctx` default 64; `--kv-page` default 16). `--prefill-chunk N` interleaves long prefills with decode. `--decode-first` holds leftover prefill while any live sequence is already decoding. `--slo-reject` / `--ttft-slo-ns` drop a waiter whose gpu-sim queue wait meets the TTFT budget (`--expert-sim`). `--itl-slo-ns` counts later-token ITL misses (does not drop). `--cuda-graphs` / `--graph-update` / `--graph-clone` / `--graph-build` / `--graph-piecewise` / `--graph-mem` / `--graph-auto-free` / `--timing-events` are the SimulatedGpuStore CUDA-graph knobs. `--mapped` / `--managed` / `--vmm` / `--vmm-retain` / `--vmm-handle` / `--vmm-page N` choose miss-page placement (default pinned). `--host-func` / `--blocking-streams` / `--sync-alloc` / `--ipc` / `--mempool` / `--mempool-max N` / `--shareable` / `--share-ptr` / `--pageable` / `--accessed-by` / `--legacy-null` / `--stream-priority` / `--seq-streams` / `--kv-sim` / `--kv-bytes N` / `--decode-priority` / `--cooperative` / `--pdl` / `--l2-persist` / `--cluster N` / `--preferred-cluster N` / `--cluster-spread` / `--cluster-load-balance` / `--max-shared` / `--max-l1` / `--non-portable-cluster` / `--sync-policy` / `--mem-sync-domain` / `--mem-sync-map` / `--mem-sync-launch` / `--mem-sync-launch-map` / `--shared-mem` / `--portable-cluster` / `--optin-shared` / `--dynamic-shared` / `--portable-shared` / `--nvlink-util` / `--device-launch` / `--device-updatable` / `--compute-slots N` / `--decode-sms N` are the remaining `GpuStoreCfg` knobs. `--prefetch` / `--plan-window` / `--plan-threshold` are the same Stay vs Fetch predictor as `gguf_gemv engine` (`--engine`; default `both` / ungated). `--trace-out FILE` appends batched MoE JSONL. `"stream": true` is chunked NDJSON token lines then a final `generated` object (`--engine` only; default serve ignores it). `--expert-slots` / `--expert-sim` park DirectStore, CachedStore, or SimulatedGpuStore on that Engine. Empty prompt and a missing GGUF file fail cleanly. No tok/s. Kernel Integrity has not signed it.

Workspace lints deny unwrap/panic/indexing/wrap-casts/`std::fs::{read,write}`. File I/O is `File` + `Read`/`Write`.

Tests write GGUFs (F32 + F16 + BF16 + Q6_K + Q5_K + Q5_1 + Q5_0 + Q4_K + Q4_1 + Q3_K + Q2_K + IQ1_M + IQ1_S + IQ2_XXS + IQ2_XS + IQ2_S + IQ3_XXS + IQ3_S + IQ4_NL + IQ4_XS + MXFP4 + NVFP4 + Q1_0 + Q2_0 + Q8_1 + TQ1_0 + TQ2_0, quantized embeddings, QKV bias, F16 1-D norms/bias, missing rope dim, mistral/phi3/gemma/qwen3/llama4/qwen2moe/qwen3moe/qwen2vl/qwen3vl/qwen3next/qwen35/phi2/bloom/gemma2/gemma3/gemma3n/gemma4 prefixes, official Gemma4 PLE, official Gemma4 MoE+PLE, official Gemma4 fused `ffn_gate_up_exps`, official Gemma4 fused plus PLE, official Gemma4 shared KV, official Gemma4 mixed SWA/global head dims, official Gemma4 SWA RoPE base, official Gemma4 omitted `wv` (K as V), official Gemma4 layer output scale, official Gemma4 proportional `rope_freqs`, official Gemma4 per-expert `ffn_down_exps.scale`, official Gemma4 per-expert `ffn_gate_exps.scale` / `ffn_up_exps.scale`, official dense `{1}` `ffn_down.scale`, official dense `{1}` `ffn_gate.scale`, official dense `{1}` `ffn_up.scale`, official dense `{1}` `attn_q.scale`, official dense `{1}` `attn_output.scale`, official dense `{1}` `attn_k.scale`, official dense `{1}` `attn_v.scale`, official NVFP4 `{1}` `output.scale`, official fused `{1}` `attn_qkv.scale`, official shared-expert `{1}` `ffn_down_shexp.scale`, official shared-expert `{1}` `ffn_gate_shexp.scale`, official shared-expert `{1}` `ffn_up_shexp.scale`, official llama MoE `n_expert>0` on `llama.*` KV, official Qwen2MoE `qwen2moe.*` KV, official Qwen3MoE `qwen3moe.*` KV, official Qwen2VL `qwen2vl.*` KV plus `rope.dimension_sections`, official Qwen3VL `qwen3vl.*` KV plus QK-Norm and `rope.dimension_sections`, official Qwen3Next `qwen3next.*` KV plus gated Q, `post_attention_norm`, MoE `norm_w` and shared expert, official Qwen35 `qwen35.*` KV plus gated Q, IMROPE, `post_attention_norm`, and dense SwiGLU, official Phi2 `phi2.*` KV plus LayerNorm, NEOX RoPE, Q-scale, and parallel GELU-seq FFN, official Bloom `bloom.*` KV plus `token_embd_norm`, fused QKV, ALiBi, and sequential GELU-seq FFN, official Gemma2 `gemma2.*` KV plus `post_attention_norm` / `post_ffw_norm`, SWA, and tanh softcap, official Gemma3 `gemma3.*` KV plus QK-Norm, `post_attention_norm` / `post_ffw_norm`, SWA period 6, and no attn softcap, official Gemma3n `gemma3n.*` KV plus AltUp / Laurel / per-layer inputs / V RMSNorm / gaussian_topk / SWA period 5 / final softcap 30, official Gemma4 `gemma4.*` KV plus QK-Norm, post-norms, V RMSNorm, attn scale 1.0, convert bool-array SWA, tied omit of `output.weight`, official Gemma4 MoE on the same `gemma4.*` KV with shared GeGLU plus GELU experts, official Gemma4 MoE+PLE on the same `gemma4.*` KV, official Gemma4 fused `ffn_gate_up_exps` on the same `gemma4.*` KV, official Gemma4 fused plus PLE on the same `gemma4.*` KV), load them, compare logits to an independent scalar of the same ggml/Llama-family math on those bytes (Gemma: embed `sqrt(n_embd)` + `ggml_gelu`; Qwen3: per-head QK-Norm; Llama4: iRoPE/NoPE + unweighted QK-Norm after RoPE + expert FFN; official llama MoE: softmax then top-k, weights after SwiGLU with `norm_w` clamp `2^-14`; official Qwen2MoE: softmax then top-k without `norm_w`, shared expert gated by `silu(x)/x`; official Qwen3MoE: Qwen3 QK-Norm plus softmax then top-k with `norm_w` clamp `2^-14`, no shared expert; official Qwen2VL: Qwen2 plus `ggml_rope_multi` m-RoPE; official Qwen3VL: Qwen3 QK-Norm plus `ggml_rope_multi` IMROPE; official Qwen3Next: gated full attention + `post_attention_norm` + MoE `norm_w` + sigmoid-gated shared expert; official Qwen35: gated full attention + IMROPE + `post_attention_norm` + dense SwiGLU; official Phi2: LayerNorm + NEOX RoPE + Q-scale + parallel GELU-seq; official Bloom: `token_embd_norm` + fused QKV + ALiBi + sequential GELU-seq; official Gemma2: post-norms + SWA + tanh softcap; official Gemma3: QK-Norm + post-norms + SWA period 6 + no attn softcap; official Gemma3n: AltUp + Laurel + V RMSNorm + attn scale 1.0 + gaussian_topk + SWA period 5 + final softcap 30; official Gemma4: QK-Norm + post-norms + V RMSNorm + attn scale 1.0 + convert bool-array SWA; official Gemma4 MoE: shared GeGLU plus custom-router GELU experts; official Gemma4 MoE+PLE: MoE then PLE inject after the FFN residual; official Gemma4 fused experts: one GEMV of twice `n_ff` rows then split gate then up; official Gemma4 fused plus PLE: fused experts then PLE inject; official Gemma4 shared KV: skip K/V on shared layers and attend against the SWA donor; official Gemma4 mixed SWA/global head dims: SWA `key_length_swa` then global `key_length`, cache stride is the max head dim; official Gemma4 SWA RoPE base: `rope.freq_base_swa` on SWA vs `rope.freq_base` on global; official Gemma4 omitted `wv`: K as V before unweighted V RMSNorm; official Gemma4 `layer_output_scale`: broadcast `ggml_mul` after the FFN residual; official Gemma4 `rope_freqs`: `theta/ff` on global layers, convert `1` then `1e30` packing; official Gemma4 `ffn_down_exps.scale`: per-expert `ggml_mul` after the down expert GEMM; official Gemma4 `ffn_gate_exps.scale` / `ffn_up_exps.scale`: per-expert `ggml_mul` after the gate/up expert GEMM, fused applies `up_exps_s` to both halves; official dense `ffn_down.scale`: `{1}` `ggml_mul` after the down GEMM; official dense `ffn_gate.scale`: `{1}` `ggml_mul` after the gate GEMM before SiLU; official dense `ffn_up.scale`: `{1}` `ggml_mul` after the up GEMM; official dense `attn_q.scale`: `{1}` `ggml_mul` after the Q GEMM; official dense `attn_output.scale`: `{1}` `ggml_mul` after the output GEMM; official dense `attn_k.scale`: `{1}` `ggml_mul` after the K GEMM; official dense `attn_v.scale`: `{1}` `ggml_mul` after the V GEMM; official NVFP4 `output.scale`: `{1}` `ggml_mul` after the NVFP4 LM-head GEMM; official fused `attn_qkv.scale`: `{1}` `ggml_mul` after the fused QKV GEMM before bias; official shared-expert `ffn_down_shexp.scale`: `{1}` `ggml_mul` after the shared-expert down GEMM; official shared-expert `ffn_gate_shexp.scale`: `{1}` `ggml_mul` after the shared-expert gate GEMM before SiLU; official shared-expert `ffn_up_shexp.scale`: `{1}` `ggml_mul` after the shared-expert up GEMM before the gated product) (including multi-token prefill, prefix KV reuse via `Llama::prompt`, paged KV intern via `Llama::new_paged_cache` / `Model::session_paged`, F16 `ggml_fp16_to_fp32` for 2-D weights and 1-D norms/bias, BF16 `dequantize_row_bf16`, Q2_K `dequantize_row_q2_K`, Q3_K `dequantize_row_q3_K`, Q4_1 `dequantize_row_q4_1`, Q5_0 `dequantize_row_q5_0`, Q5_1 `dequantize_row_q5_1`, Q5_K `dequantize_row_q5_K`, IQ1_M `dequantize_row_iq1_m`, IQ1_S `dequantize_row_iq1_s`, IQ2_XXS `dequantize_row_iq2_xxs`, IQ2_XS `dequantize_row_iq2_xs`, IQ2_S `dequantize_row_iq2_s`, IQ3_XXS `dequantize_row_iq3_xxs`, IQ3_S `dequantize_row_iq3_s`, IQ4_NL `dequantize_row_iq4_nl`, IQ4_XS `dequantize_row_iq4_xs`, MXFP4 `dequantize_row_mxfp4`, NVFP4 `dequantize_row_nvfp4`, Q1_0 `dequantize_row_q1_0`, Q2_0 `dequantize_row_q2_0`, Q8_1 `q*d`, TQ1_0 `dequantize_row_tq1_0`, and TQ2_0 `dequantize_row_tq2_0`), then encode/greedy/decode. Sampler tests compare temperature / top-k / top-p / repeat penalty to an independent candidate-list oracle of the same math, plus SplitMix64 published outputs. Tokenizer tests cover GPT-2 `Ċ`/`Ġ` piece decode, `<0xHH>` byte-fallback, SentencePiece `▁`, HashMap `token_id`, and the writer-built tiny merge. Serve tests cover CLI parse, loopback bind, HTTP/1.1 framing, JSON request/response shape, empty prompt, tiny-model `POST /generate` vs `greedy_generate_ctx`, persistent-cache `prefix_hit`, `--kv-page` identity, `--engine` concurrent posts sharing a prefill GEMM, DirectStore MoE acquires, intern `page_hits` across sequential Engine HTTP prompts, `--prefill-chunk`, `--trace-out` MoE JSONL, `"stream": true` chunked NDJSON, `GET /health` / `GET /v1/models` / `GET /metrics`, `POST /tokenize` / `POST /detokenize` (no 2GB GGUF).
