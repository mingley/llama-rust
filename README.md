...
// Decode tokens on the shared pool GEMM together (`Llama::forward_batch`).
...
`cargo run -p llama-rust --example session` runs the writer-built Qwen3MoE tiny through CachedStore. `cargo run -p llama-rust --example engine` runs two sequences on `Engine` (chunked prefill, intern hits, recompute preemption). `gguf_gemv engine` is the same scheduler from the binary.
