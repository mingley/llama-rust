# Stopped 2026-08-27 — IQ2_XXS 2-D weights
...
- GGUF v3: F32, F16, Q4_0, Q8_0, Q4_K, Q5_K, Q6_K, Q8_K, IQ2_XXS, IQ2_S, IQ3_XXS, IQ3_S, IQ4_NL, IQ4_XS. Kernels read on-disk bytes (no private f32-scale copy).
...
- IQ2_XXS is `GGML_TYPE_IQ2_XXS` = 16 (66-byte `block_iq2_xxs`). Next remaining IQ* type that common OSS `*-IQ2_XXS.gguf` files actually have (bartowski / mradermacher standalone). Writer-built tiny uses IQ2_XXS for 2-D weights (`token_embd`, `output`, attn/ffn); 1-D norms stay F32. Load/GEMV/GEMM/embed logits match an independent scalar of the same ggml `dequantize_row_iq2_xxs` walk (`d*(0.5+ls)*0.25*iq2xxs_grid[q]*ksigns`). No tok/s.
...
- Q4_K_M shape that common OSS files actually have:
  - quantized `token_embd.weight` (Q4_K / Q5_K / Q6_K / IQ2_XXS / IQ2_S / IQ3_XXS / IQ3_S / IQ4_NL / IQ4_XS / F32) or F16
...
Nothing. This slice is STATUS item 2 (IQ2_XXS). Metal-in-crate was skipped: this Linux VM cannot compile or run Metal.
...
2. **Dtypes / arches still rejected.** Remaining IQ* (IQ2_XS, IQ1_S, IQ1_M), Gemma, MoE, vision, Qwen3, Llama4. 1-D F16 norms/bias still rejected. Tied `output.weight` (reuse `token_embd`) untested.
