# Stopped 2026-08-28 — Gemma architecture
...
- Decode: RMSNorm, RoPE, GQA+KV, SwiGLU (llama/qwen2/mistral/phi3) or Gemma GeGLU, lm_head, greedy sample by default. Gemma scales token embeds by `sqrt(n_embd)` (`ggml_scale`). RMSNorm +1 is a convert-hf bake on GGUF `norm.weight` bytes; decode uses `LLM_NORM_RMS` as-is.
...
- Architectures: `llama`, `qwen2`, `mistral`, `phi3`, `gemma` `{arch}.*` KV. `gemma2` stays rejected.
...
Nothing. This slice is STATUS item 2 (Gemma architecture). Metal-in-crate was skipped: this Linux VM cannot compile or run Metal.
...
2. **Dtypes / arches still rejected.** MoE, vision, Qwen3, Llama4. `gemma` `{arch}.*` KV loads (writer-built tiny). `gemma2` is still rejected (different official decode family). 1-D F16 norms/bias load (attn/ffn/`output_norm` and optional `attn_{q,k,v}.bias`). Tied `output.weight` reuses `token_embd.weight` when absent. Common OSS IQ* 2-D, BF16 2-D, Q2_K 2-D, Q3_K 2-D, Q4_1 2-D, Q5_0 2-D, Q5_1 2-D, MXFP4 2-D, NVFP4 2-D, Q1_0 2-D, Q2_0 2-D, Q8_1 2-D, TQ1_0 2-D, and TQ2_0 2-D are loaded (IQ1_M / IQ1_S / IQ2_XXS / IQ2_XS / IQ2_S / IQ3_XXS / IQ3_S / IQ4_NL / IQ4_XS / BF16 / Q2_K / Q3_K / Q4_1 / Q5_0 / Q5_1 / MXFP4 / NVFP4 / Q1_0 / Q2_0 / Q8_1 / TQ1_0 / TQ2_0). IQ4_NL_4_4/4_8/8_8 (36..=38) are ggml-removed, not a remaining hole. Next remaining live rejected ggml weight type id: none. Next remaining item-2 hole: MoE / vision / Qwen3 / Llama4. Do not invent a dtype.
...
Next code change should be item 1 (Metal-in-crate) on a machine that can compile Metal, or remaining item-2 work (MoE / vision / Qwen3 / Llama4). Do not add crates.io runtime deps or `unsafe`. Do not start Metal-in-crate on Linux. Do not invent a `block_iq4_nl_4_4` dequant.
