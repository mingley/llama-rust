...
521. [x] `gpu-sim` CUDA `cudaGraphInstantiateFlagDeviceLaunch` cannot
    instantiate an empty graph and requires at least one kernel, memcpy,
    or memset node ([`instantiate_graph_with_flags`] /
    [`instantiate_graph_with_params`]; Invalid `"device launch empty"`;
    [`GraphInstantiateResult::NodeOperationNotSupported`], `err_node =
    None`). Empty, batch-mem, wait-value, and write-value nodes stay
    `NodeOperationNotSupported` (`"device launch instantiate flag"`).
    Child graphs stay refused. Kernel, memcpy, and memset-only graphs
    stay legal. `DEVICE_LAUNCH` plus `UPLOAD` stays. Capture is still
...
523. [x] `gpu-sim` CUDA DeviceLaunch memset dest must be origin-device
    memory or pinned/mapped host ([`instantiate_graph_with_flags`] /
    [`instantiate_graph_with_params`]; off-device, pageable, or unknown
    dest is `NodeOperationNotSupported` `"device launch instantiate
    flag"`). Host instantiate of that memset stays. Capture is still
    reported first. Origin-device malloc, pinned, and mapped DeviceLaunch
    memset stay legal. This VM does not invent Engine
...
537. [x] `gpu-sim` CUDA `cudaGraphExecUpdate` of an exec with an
    in-flight host `launch_graph` or `upload_graph_async` is Invalid
    `"exec in flight"`. Capture is still reported first. After
    synchronize, update stays. Host SetParams of that exec stay.
    DeviceLaunch update stays NotSupported. This VM does not invent
...
539. [x] `gpu-sim` CUDA queries of an exec whose handle was destroyed
    while a launch was in flight are Invalid `"unknown graph"`
    (`graph_len` / `graph_instantiated` / `graph_get_id` /
    `graph_nodes` / `graph_debug_dot`). The launch still finishes.
    Capture is still reported first. Getters before destroy stay.
    Definition queries stay. This VM does not invent Engine
...
541. [x] `gpu-sim` CUDA definition mutators of an exec whose handle was
    destroyed while a launch was in flight are Invalid `"unknown graph"`
    before `"graph instantiated"` (add node, user-object retain/release,
    destroy node, remove deps, conditional create). Capture is still
    reported first. Live exec still `"graph instantiated"`. Definition
    mutators stay. This VM does not invent Engine `--graph-def-gone`.
    `gpu-profile capture` is still refused. Dual score still has no
    `$/M tokens`.

542. [x] `gpu-sim` CUDA `begin_capture_to_graph` of an exec whose handle
...
543. [x] `gpu-sim` CUDA `cudaGraphExecUpdate` whose source graph handle was
...
544. [x] `gpu-sim` CUDA `cudaGraphKernelNodeCopyAttributes` of an exec whose
    handle was destroyed while a launch was in flight is Invalid
    `"unknown graph"` (source or dest). Capture is still reported first.
    Live exec as either end stays. Definition CopyAttributes stays. Exec
    CopyAttributes of a parked dest stays unknown via `as_exec`. This VM
...
547. [x] `gpu-sim` CUDA `cudaGraphAddChildGraphNode` /
    ...
    stays. Add-child of the definition after that exec is parked stays.
...
549. [x] `gpu-sim` CUDA `cudaGraphNodeGetDependencies` /
...
629. [ ] Next numbered PLAN item after 628 is the next `gpu-sim` / Engine /
    ...
    (still ignored).
    Do not invent a second DeviceLaunch empty-graph check or Engine
    `--device-launch-empty`. Do not reverse DeviceLaunch child-graph
    ...
    Do not invent a second DeviceLaunch memset-dest check or Engine
    ...
    Do not invent a second in-flight ExecUpdate refuse or Engine
    ...
    (a new exec). Do not invent a second parked-exec definition-mutator
    unknown-before-instantiated check or Engine `--graph-def-gone`. Do not
    reverse retain illegal on a live exec. Do not reverse add-node illegal
    on a live exec. Do not invent a second parked-exec capture-to-graph
    unknown check or Engine `--graph-capture-gone`. Do not reverse live
    exec capture-to-graph `"graph instantiated"`. Do not reverse
    capture-to-graph of the definition after the primary exec is parked.
    Do not invent a second ExecUpdate parked-source unknown check or Engine
    `--graph-update-src-gone`. Do not reverse live exec as ExecUpdate
    source. Do not reverse definition as ExecUpdate source while an exec
    is parked. Do not reverse dest in-flight `"exec in flight"`.
    Do not invent a second CopyAttributes parked-handle unknown check or
    ...
    Do not invent a second parked-child-handle unknown check or Engine
    ...
    Do not invent a second GetDependencies parked-handle unknown check or
