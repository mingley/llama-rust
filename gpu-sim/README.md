...
semaphore interop is not modeled). `MemDecompressAlgorithmMask` is
always 0 (hardware decompress is not modeled).
...
profile name). `device_total_mem` is `cuDeviceTotalMem` (HBM bytes).
