...
source physicals (no extra HBM). `ipc_get_mem_handle` is `cuIpcGetMemHandle` (identity with `ipc_get`). Host-sync; capture refused. No Engine `--ipc-get-mem-handle`. `ipc_open_with_flags` accepts
