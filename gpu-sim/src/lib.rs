...
//! include create / split-flags / bind / record / wait / sync. `expertvm sim --green-ctx` /
...
//! `expertvm sim --copy-host` / `gguf_gemv engine --expert-sim --copy-host`
...
//! parameter). Add is illegal on an instantiated exec and during capture.
...
mod tests {
    ...
    #[test]
    fn graph_add_memset_between_alloc_and_kernel_serializes() {
        ...
        let run = |memset: bool| {
            ...
            let mut prev = 0usize;
            if memset {
                sim.graph_add_memset(g, KernelBuf::whole(scratch)).unwrap();
    ...
    #[test]
    fn green_ctx_synchronize_does_not_wait_other_ctx() {
        let d = DeviceId(0);
        let kind_short = KernelKind::other(8, 8);
        let kind_long = KernelKind::other(1 << 40, 8);
        let mut sim = Sim::new(h100());
        ...
        sim.green_ctx_stream_create(c1, StreamId(2), StreamCreateFlags::NON_BLOCKING, 0)
            .unwrap();
        enq(sim.kernel(d, kind_short, &[a], &[a], StreamId(1)));
        enq(sim.kernel(d, kind_long, &[a], &[a], StreamId(2)));
        let t0 = sim.clock_ns();
        sim.green_ctx_synchronize(c0).unwrap();
        let ctx_ns = sim.clock_ns().saturating_sub(t0);
        assert!(
            !sim.stream_is_idle(d, StreamId(2)).unwrap(),
            "cudaExecutionCtxSynchronize must leave the other green ctx running"
        );
        sim.synchronize_device(d).unwrap();
        let device_ns = sim.clock_ns().saturating_sub(t0);
        assert!(
            device_ns > ctx_ns,
            "device sync must wait the other ctx; ctx={ctx_ns} device={device_ns}"
        );
        sim.begin_capture(d, StreamId(1)).unwrap();
        let err = sim.green_ctx_synchronize(c0).unwrap_err();
        assert!(
            format!("{err:?}").contains("cannot capture green ctx sync"),
            "{err:?}"
        );
        let _g = sim.end_capture().unwrap();
        let err = sim.green_ctx_synchronize(GreenCtxId(99)).unwrap_err();
        assert!(format!("{err:?}").contains("unknown green ctx"), "{err:?}");
    }
    ...
    #[test]
    fn graph_get_id_matches_debug_dot_handles() {
        let mut sim = Sim::new(h100());
        let d = DeviceId(0);
        let s = StreamId(0);
        let a = sim.malloc(d, 4096).unwrap();
        let g0 = sim.create_graph(d, s).unwrap();
        let g1 = sim.create_graph(d, s).unwrap();
        let id0 = sim.graph_get_id(g0).unwrap();
        let id1 = sim.graph_get_id(g1).unwrap();
        assert_ne!(id0, id1);
        assert_eq!(id0, g0.0);
        sim.graph_add_kernel(g0, KernelKind::other(8, 8), &[a], &[a])
            .unwrap();
        let handles = sim
            .graph_debug_dot_with_flags(g0, GraphDebugDotFlags::HANDLES)
            .unwrap();
        assert!(handles.contains(&format!("digraph g{id0}")), "{handles}");
        sim.begin_capture(d, s).unwrap();
        assert_eq!(sim.graph_get_id(g0).unwrap(), id0);
        let _end = sim.end_capture().unwrap();
        let exec = sim.instantiate_graph(g0).unwrap();
        let exec_id = sim.graph_get_id(exec).unwrap();
        assert_ne!(exec_id, id0);
        let clone = sim.clone_graph(g0).unwrap();
        assert_ne!(sim.graph_get_id(clone).unwrap(), id0);
        assert_ne!(sim.graph_get_id(clone).unwrap(), exec_id);
        match sim.graph_get_id(GraphId(99)) {
            Err(SimError::Invalid { why }) => assert!(why.contains("unknown graph"), "{why}"),
            other => panic!("{other:?}"),
        }
        sim.destroy_graph(g1).unwrap();
        match sim.graph_get_id(g1) {
            Err(SimError::Invalid { why }) => assert!(why.contains("unknown graph"), "{why}"),
            other => panic!("{other:?}"),
        }
    }
    ...
    #[test]
    fn graph_node_get_tools_id_is_unique_across_graphs() {
        let mut sim = Sim::new(h100());
        let d = DeviceId(0);
        let s = StreamId(0);
        let a = sim.malloc(d, 4096).unwrap();
        ...
        sim.graph_add_kernel(g, KernelKind::other(8, 8), &[a], &[a])
            .unwrap();
        sim.graph_add_empty(g).unwrap();
        let t0 = sim.graph_node_get_tools_id(g, 0).unwrap();
        let t1 = sim.graph_node_get_tools_id(g, 1).unwrap();
        assert_ne!(t0, t1);
        assert_eq!(t0, sim.graph_node_get_tools_id(g, 0).unwrap());
        assert_ne!(t0, u64::from(sim.graph_node_get_local_id(g, 0).unwrap()));
        assert_ne!(t0, u64::from(sim.graph_get_id(g).unwrap()));
        sim.begin_capture(d, s).unwrap();
        assert_eq!(sim.graph_node_get_tools_id(g, 0).unwrap(), t0);
        let _end = sim.end_capture().unwrap();
        let exec = sim.instantiate_graph(g).unwrap();
        let exec_t0 = sim.graph_node_get_tools_id(exec, 0).unwrap();
        assert_ne!(exec_t0, t0);
        let clone = sim.clone_graph(g).unwrap();
        let clone_t0 = sim.graph_node_get_tools_id(clone, 0).unwrap();
        assert_ne!(clone_t0, t0);
        assert_ne!(clone_t0, exec_t0);
        match sim.graph_node_get_tools_id(GraphId(99), 0) {
            Err(SimError::Invalid { why }) => assert!(why.contains("unknown graph"), "{why}"),
            other => panic!("{other:?}"),
        }
        match sim.graph_node_get_tools_id(g, 9) {
            Err(SimError::Invalid { why }) => assert!(why.contains("unknown graph node"), "{why}"),
            other => panic!("{other:?}"),
        }
        sim.graph_destroy_node(g, 0).unwrap();
        match sim.graph_node_get_tools_id(g, 0) {
            Err(SimError::Invalid { why }) => assert!(why.contains("unknown graph node"), "{why}"),
            other => panic!("{other:?}"),
        }
    ...
    #[test]
    fn device_get_attribute_global_l1_cache_unsupported() {
        let mut sim = Sim::new(h100());
        let d = DeviceId(0);
    ...
    #[test]
    fn device_get_attribute_texture_alignment_is_zero() {
        let mut sim = Sim::new(h100());
        let d = DeviceId(0);
        let hp = sim.device_get_properties(d).unwrap();
    ...
    #[test]
    fn device_get_attribute_texture_pitch_alignment_is_zero() {
        let mut sim = Sim::new(h100());
        let d = DeviceId(0);
        let hp = sim.device_get_properties(d).unwrap();
    ...
    #[test]
    fn graph_memcpy_set_params_2d_requires_is_2d() {
        let mut sim = Sim::new(h100());
        let d = DeviceId(0);
        let s = StreamId(0);
        let (a, pitch) = sim.malloc_pitch(d, 256, 8).unwrap();
        ...
        sim.graph_memcpy_set_params_2d(g, 0, &patched).unwrap();
        assert_eq!(sim.graph_memcpy_get_params(g, 0).unwrap().height, 4);
        match sim.graph_memcpy_set_params_2d(
            g,
            0,
            &MemcpyOp {
                height: 1,
        ...
        let exec = sim.instantiate_graph(g).unwrap();
        sim.graph_exec_memcpy_set_params_2d(exec, 0, &op).unwrap();
        assert_eq!(sim.graph_exec_memcpy_get_params(exec, 0).unwrap().height, 8);
        match sim.graph_exec_memcpy_set_params_2d(
            exec,
            0,
            &MemcpyOp {
                height: 1,
    ...
    #[test]
    fn pool_set_access_n_is_cuda_mempool_set_access_descs() {
        let mut sim = Sim::new(HardwareProfile::example_8xh100_nvlink());
        let d0 = DeviceId(0);
        let d1 = DeviceId(1);
        let d2 = DeviceId(2);
        let p0 = sim.default_pool(d0).unwrap();
        sim.pool_set_access_n(p0, &[]).unwrap();
        assert!(!sim.is_pool_accessed_by(p0, d1).unwrap());
        ...
        .unwrap();
        assert!(sim.is_pool_accessed_by(p0, d1).unwrap());
        assert!(sim.is_pool_accessed_by(p0, d2).unwrap());
        sim.pool_set_access_n(
            p0,
            &[MemAccessDesc {
                location: Place::Device(d1),
        ...
        assert!(sim.is_pool_accessed_by(p0, d2).unwrap());
        sim.begin_capture(d0, StreamId(0)).unwrap();
        match sim.pool_set_access_n(p0, &[]) {
            Err(SimError::Invalid { why }) => assert!(why.contains("capture"), "{why}"),
            other => panic!("{other:?}"),
        }
        let _g = sim.end_capture().unwrap();
    ...
    #[test]
    fn graph_add_set_conditional_then_if_enables_body() {
        let long = KernelKind::other(1 << 40, 4096);
        let run = |value: u32| {
            let mut sim = Sim::new(h100());
            let d = DeviceId(0);
            ...
            let g = sim.create_graph(d, s).unwrap();
            let h = sim.graph_conditional_create(g, 0).unwrap();
            sim.graph_add_set_conditional(g, h, value).unwrap();
            let body = sim.graph_add_if(g, h).unwrap();
            sim.graph_add_kernel(body, long.clone(), &[a], &[a])
                .unwrap();
            sim.graph_add_dependencies(g, 0, 1).unwrap();
            ...
            assert_eq!(sim.graph_set_conditional_nodes(g).unwrap().len(), 1);
            let _ = sim.instantiate_graph(g).unwrap();
            sim.upload_graph(g).unwrap();
            let t0 = sim.clock_ns();
            let n = sim.launch_graph(g, s).unwrap();
        ...
        assert!(
            skipped < enabled,
            "graph_add_set_conditional(1) must run the IF body; set={enabled} skip={skipped}"
        );
    }

    #[test]
    fn graph_add_set_conditional_rejects_instantiated_capture_and_mismatch() {
        let mut sim = Sim::new(h100());
        let d = DeviceId(0);
        let s = StreamId(0);
        let g = sim.create_graph(d, s).unwrap();
        ...
        let h = sim.graph_conditional_create(g, 0).unwrap();
        let err = sim.graph_add_set_conditional(other, h, 1).unwrap_err();
        match err {
        ...
        sim.begin_capture(d, s).unwrap();
        let err = sim.graph_add_set_conditional(g, h, 1).unwrap_err();
        match err {
            SimError::Invalid { why } => assert!(why.contains("capture"), "{why}"),
            e => panic!("{e:?}"),
        }
    ...
    #[test]
    fn graph_exec_set_conditional_params_retargets_without_second_graph() {
        let long = KernelKind::other(1 << 40, 4096);
        let mut sim = Sim::new(h100());
        let d = DeviceId(0);
        let s = StreamId(0);
        ...
        assert_eq!(n0, 2);
        assert!(sim.graph_uploaded(exec).unwrap());
        let (node, handle, value) = sim.graph_unique_set_conditional(exec).unwrap();
        assert_eq!(handle, h);
        assert_eq!(value, 0);
        match sim.graph_exec_node_get_params(exec, node).unwrap() {
            GraphNodeParams::SetConditional {
                handle: got,
                value: v,
            } => {
        ...
        sim.graph_exec_set_conditional_params(exec, node, 1)
            .unwrap();
        assert!(!sim.graph_uploaded(exec).unwrap());
        let other = sim.create_graph(d, s).unwrap();
        let other_h = sim.graph_conditional_create(other, 0).unwrap();
        let err = sim
            .graph_exec_node_set_params(
                exec,
                node,
                GraphNodeParams::SetConditional {
                    handle: other_h,
    ...
    fn device_launch_rejects_mem_event_child_and_host() {
        ...
        sim.graph_add_set_conditional(cond, h, 1).unwrap();
        let err = sim
            .instantiate_graph_with_flags(cond, GraphInstantiateFlags::DEVICE_LAUNCH)
            .unwrap_err();
        match err {
            SimError::Invalid { why } => assert!(why.contains("device launch"), "{why}"),
            other => panic!("{other:?}"),
