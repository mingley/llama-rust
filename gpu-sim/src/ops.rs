...
pub struct DeviceProperties {
    ...
    /// `cudaDevAttrSurfaceAlignment` ([`DeviceAttr::SurfaceAlignment`]). Always
    ...
    /// `cudaDevAttrMaxTexture1DWidth` ([`DeviceAttr::MaxTexture1DWidth`]).
...
pub struct KernelAttrs {
    ...
    /// CUDA: once true, SetAttribute to false is Invalid; the node cannot be
    /// destroyed; CopyAttributes cannot involve it; the graph cannot be
    /// instantiated twice or passed to `cudaGraphExecUpdate`. Decode identity
    /// stays `false`.
...
pub struct GraphAddNode {
    ...
    /// Then-body / WHILE body (`cudaGraphConditionalNodeParams::phGraph_out[0]`).
